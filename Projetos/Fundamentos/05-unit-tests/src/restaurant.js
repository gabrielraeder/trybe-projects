...
const createMenu = (objeto) => {
  const obj = { fetchMenu: () => objeto };
  obj.consumption = [];
  obj.order = (request) => obj.consumption.push(request);
  obj.pay = () => {
    const consum = obj.consumption;
    let total = 0;
    const novo = {};
    Object.assign(novo, objeto.food, objeto.drink);
    for (let index = 0; index < consum.length; index += 1) {
      if (consum[index] in novo) {
        total += novo[consum[index]];
      }
    }
    return total * 1.1;
  };
